...
export default class PasswordStrengthCheck extends Component {
    ...
    return (
      <View style={{flexDirection:'row'}}>
        <TextInput
          ...
          onFocus={() => this.setState({ isSecureTextEntry: false })}
          onBlur={() => this.setState({ isSecureTextEntry: true })}
        />
      </View>
    );
  }
  ...
  renderPasswordStrength() {
    ...
    console.log("strengthLevels.length", strengthLevels.length)
    
    // make sure level doesn't exceed the strengthLevels
    if(level >= strengthLevels.length) {
    ...
    if (level !== -1) {
      ...
      if (tooShort && tooShort.enabled && this.state.isTooShort) {
