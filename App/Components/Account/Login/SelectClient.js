...
export class LoginSelectClient extends React.Component {
  ...
  componentDidMount() {
    ...
    let sites = navigation.getParam('sites', [] );

    let selectedClient = navigation.getParam('selectedClient', '' );
  ...
  matchSites = (query) => {
    ...
    let filtered = this.state.sites.filter(item => item.toLowerCase().indexOf(query.toLowerCase()) !== -1)
...
export default LoginSelectClient;